...
hyperparams = {	'SBM': {'sage':{'influence_thresh': 2.08, 'switch_k': 16},\
					  'gcn':{'influence_thresh': 1.7, 'switch_k': 16}},
				
				'polblogs': {'sage':{'influence_thresh': 0.14, 'switch_k': 4},\
					  'gcn':{'influence_thresh': 0.4, 'switch_k': 4}},
				
                'cora_2comm':{'sage':{'influence_thresh': 2, 'switch_k': 8},\
				'gcn':{'influence_thresh': 2, 'switch_k': 8}},

                'CoauthorCS': {'sage':{'influence_thresh': 2, 'switch_k': 8},\
                'gcn':{'influence_thresh': 2, 'switch_k': 8}},
				
				'citeseer_2comm': {'sage':{'influence_thresh': 1, 'switch_k': 8},\
					  'gcn':{'influence_thresh': 0.5, 'switch_k': 8}}}
...
class Synth_Dataset():
    def __init__(self,name):
        self.name = name
        with open('./synth_data/{}.pkl'.format(name),'rb') as f:
            self.adj, self.features, self.labels = pickle.load(f)        
        self.idx_train, self.idx_val, self.idx_test = get_train_val_test(nnodes=self.adj.shape[0],stratify=self.labels,seed=1) 
